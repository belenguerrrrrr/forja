...
export async function POST() {
  try {
    ...
    // Obtener plan activo y datos del usuario
    ...
    // Obtener progreso últimas 2 semanas
    ...
    const progressData = {
      ...
      weight_change: weights?.length >= 2 ? weights[weights.length - 1].weight - weights[0].weight : null,
    }

    // Generar ajuste con Claude
    const adjustment = await generatePlanAdjustment(userData, progressData)

    // Aplicar ajuste al plan actual
