...
/**
 * Genera plan de preparación para un evento específico (carrera, maratón, etc.)
 */
export async function generateEventPlan(userData, eventData) {
  const { event_type, event_date, current_level } = eventData
  const weeksUntilEvent = Math.ceil((new Date(event_date) - new Date()) / (7 * 24 * 60 * 60 * 1000))

  const prompt = `Genera un plan de preparación de ${weeksUntilEvent} semanas para este objetivo específico.

USUARIO:
- Peso: ${userData.current_weight}kg
- Nivel actual: ${current_level}
- Días disponibles: ${userData.training_days_per_week}/semana
- Duración sesión: ${userData.training_duration_minutes} min

EVENTO:
- Tipo: ${event_type}
- Fecha: ${event_date}
- Semanas disponibles: ${weeksUntilEvent}

Genera un plan semanal progresivo con periodización correcta (base, construcción, pico, tapering).
Responde ÚNICAMENTE con JSON: { "weeks": [ { "week": 1, "phase": "base|build|peak|taper", "focus": "...", "sessions": [...], "volume_km_or_sets": 0, "intensity": "low|medium|high" } ] }`
